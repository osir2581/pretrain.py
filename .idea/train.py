...
def arg_parser():
    ...
    parser.add_argument("--total_iter", default = 5000, type = int)
...
def train(args):
    ...
    vgg_model = loss.Vgg19("..\\vgg19\\vgg19_no_fc.npy")
    ...
    train_writer = tf.summary.FileWriter(args.save_dir+'\\train_log')
    summary_op = tf.summary.merge_all()
    saver = tf.train.Saver(var_list=gene_vars, max_to_keep=20)
   
    with tf.device('/device:GPU:0'):
        sess.run(tf.global_variables_initializer())
        saver.restore(sess, tf.train.latest_checkpoint('..\\test_code\\saved_models'))
